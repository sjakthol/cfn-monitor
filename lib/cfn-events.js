import { paginateDescribeStackEvents } from "@aws-sdk/client-cloudformation";
...
async function* getStackEvents(stackId, region) {
  const client = helpers.getCloudFormationClient({ region });
 ...
 */
async function* streamStackEvents(stackName, stackId, region, pollInterval = 1000) {
  const seen = new Set();
  let complete = false;
  ...
  while (!complete) {
    ...
    for await (const event of getStackEvents(stackId, region)) {
      ...
      if (event.LogicalResourceId === stackName && event.ResourceType === "AWS::CloudFormation::Stack") {
