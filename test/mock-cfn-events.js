...
function mockDescribeStackEvents(eventsAddedPerPollingRound) {
  ...
  return (args) => {
    ...
    const StackEvents = [events[nextEventIndex], events[nextEventIndex + 1]].filter((e) => e);
    ...
    const resp = events[nextEventIndex] ? { StackEvents, NextToken: `next=${nextEventIndex}` } : { StackEvents };
    return resp;
  };
}
