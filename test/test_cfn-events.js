...
describe("cfn-events", function () {
  ...
  it("should poll stack events correctly", async function () {
    ...
    const events = await utils.asyncGeneratorToArray(
      cfnEvents.streamStackEvents("test-stack", "test-stack-id", "eu-north-1", 10),
    );
    expect(events.map((e) => e.EventId)).to.deep.equal(["0001", "0002", "0003", "0004", "0005", "0006"]);
  });
  ...
  it("should poll stack events correctly with default polling interval", async function () {
    ...
    const events = await utils.asyncGeneratorToArray(
      cfnEvents.streamStackEvents("test-stack", "test-stack-id", "eu-north-1"),
    );
    expect(events.map((e) => e.EventId)).to.deep.equal(["0002", "0003", "0004", "0005"]);
  });
  ...
  it("should handle cleanup states correctly", async function () {
    ...
    expect(events.map((e) => e.EventId)).to.deep.equal(["0002", "0003", "0004", "0005", "0006"]);
  });
