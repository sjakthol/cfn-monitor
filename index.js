...
async function maybeStartToMonitorStack(input) {
  ...
  const nestedStacks = [];
  for await (const e of cfnEvents.streamStackEvents(stack.StackName, stack.StackId, info.region)) {
...
async function startToMonitorInProgressStacks() {
  ...
  if (stacks.length === 0) {
    output.write(`${chalk.green("INFO")}: No stacks are being created / deleted / updated.`);
    return;
  }

  output.write(`${chalk.green("INFO")}: ${stacks.length} stack${stacks.length === 1 ? " is" : "s are"} being changed.`);
  ...
  if (!process.stdin.isTTY) {
    ...
    rl.on("line", (line) => {
      ...
      if (!isDeploy && line.match(/Waiting for stack create\/update to complete/)) {
        output.write(
          chalk.green("INFO") +
            ": The command piped to cfn-monitor " +
    ...
    if (!seenStackArns.size) {
      output.write(
        chalk.green("INFO") +
          ": No input nor stacks from the command " +
...
export { maybeStartToMonitorStack, run, startToMonitorDeletingStacks, startToMonitorInProgressStacks };
